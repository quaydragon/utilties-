...
public interface GenericListAdtNode<T> {
  ...
  /**
   * Returns the list node if the predicate operation equals true.
   * @param predicate operation to determine if value fits
   * @return empty node if false and this node if true
   */
  GenericListAdtNode<T> filter(Predicate predicate);
  
  /**
   * Fold function takes in an accumulator function and returns an identity.
   * 
   * @param identity object passed to function
   * @param accumulator method to be applied to the list node
   * @return object from accumulator 
   */
